...
{
    /// <summary>
    /// Simple script to position an object in front of the camera.
    /// The script is based on the PlaceFromCamera script in the Unity Examples.
    /// </summary>
    public class HeadPoseContent : MonoBehaviour
    {
        ...
        private float _positionSmoothTime = 0.5f;
        ...
        private bool _placeOnAwake = true;

        [SerializeField, Tooltip("Toggle to set position on update.")]
        private bool _placeOnUpdate = true;
        ...
        private Vector2 _movementThreshold = new Vector2(45f, 10f);
        ...
        private void UpdateTransform(Camera camera)
        {
            ...
            Vector3 forwardVec = (_lockToXZPlane)
                ? new Vector3(camera.transform.forward.x, 0, camera.transform.forward.z).normalized
                : camera.transform.forward;
            Vector3 targetPosition = (_useLocalSpace ? camera.transform.localPosition : camera.transform.position) +
                                     offsetVector + (forwardVec * _distance);

            if (_forceUpdate || !_useThreshold ||
                Vector3.Distance((_useLocalSpace ? transform.localPosition : transform.position), _movePosition) >
                0.01f ||
                Quaternion.Angle(
                    Quaternion.Euler(
                        (_useLocalSpace ? camera.transform.localEulerAngles.x : camera.transform.eulerAngles.x), 0, 0),
                    Quaternion.Euler(_lastCameraEuler.x, 0, 0)) > _movementThreshold.y ||
                Quaternion.Angle(
                    Quaternion.Euler(0,
                        (_useLocalSpace ? camera.transform.localEulerAngles.y : camera.transform.eulerAngles.y), 0),
                    Quaternion.Euler(0, _lastCameraEuler.y, 0)) > _movementThreshold.x ||
                Vector3.Distance((_useLocalSpace ? camera.transform.localPosition : camera.transform.position),
                    _lastCameraPosition) > _distanceThreshold)
            {
                if (_forceUpdate)
                    _forceUpdate = false;
            ...
            if (_useLocalSpace)
            {
                transform.localPosition = _placeOnAwake
                    ? targetPosition
                    : Vector3.SmoothDamp(transform.localPosition, _movePosition, ref _positionVelocity,
                        _positionSmoothTime);
            }
            else
            {
                transform.position = _placeOnAwake
                    ? targetPosition
                    : Vector3.SmoothDamp(transform.position, _movePosition, ref _positionVelocity, _positionSmoothTime);
            }
            ...
            if (_lookDirection == LookDirection.LookAwayFromCamera)
            {
                targetRotation =
                    Quaternion.LookRotation(
                        (_useLocalSpace ? transform.localPosition : transform.position) -
                        (_useLocalSpace ? camera.transform.localPosition : camera.transform.position),
                        camera.transform.up);
            }
            else if (_lookDirection == LookDirection.LookAtCamera)
            {
                targetRotation =
                    Quaternion.LookRotation(
                        (_useLocalSpace ? camera.transform.localPosition : camera.transform.position) -
                        (_useLocalSpace ? transform.localPosition : transform.position), camera.transform.up);
            }

            if (_useLocalSpace)
            {
                transform.localRotation = _placeOnAwake
                    ? targetRotation
                    : Quaternion.Slerp(transform.localRotation, targetRotation, Time.deltaTime / _rotationSmoothTime);
            }
            else
            {
                transform.rotation = _placeOnAwake
                    ? targetRotation
                    : Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime / _rotationSmoothTime);
            }
            ...
            if (_lockXRotation)
            {
                if (_useLocalSpace)
                {
                    transform.localRotation = Quaternion.Euler(0, transform.localRotation.eulerAngles.y,
